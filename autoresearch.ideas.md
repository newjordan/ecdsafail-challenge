...
- **Direct controlled-constant Solinas corrections:** `KAL_DIRECT_CONST_HALVE=1` is clean and gives `4,121,014 @ 2715q` by replacing the loaded 256q constant register in modular halve with `csub_nbit_const_direct_fast`; default remains `4,111,918 @ 2716q`. The add analogue (`KAL_DIRECT_CONST_DOUBLE=1`) failed with `altseed_classical_total=1`, `altseed_phase_batches_total=2`. If revisiting, first add a standalone basis/phase test for `cadd_nbit_const_direct_fast`, then try removing loaded constants in other Solinas cadd/csub sites.
