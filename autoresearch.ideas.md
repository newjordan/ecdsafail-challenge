...
Directly X-measuring affine slope after point-add is also not a cheap MBUC cleanup: `measuring_lambda_after_affine_add_still_needs_growing_degree_phase` corrects `λ=-(R_y+Q_y)/(R_x-Q_x)` on curve-supported outputs and sees min degrees `2,3,3,4,4` for toy n=4,6,8,10,12.
